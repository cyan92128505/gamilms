...
var server_config = require(process.cwd() + '/config/server_config.json');
...
var facebook_return = 'http://' + HOST + '/' + server_config.project_name + '/login/facebook/return';
...
// 紀錄器
fs.stat(process.cwd() + '/log/access.log', function(err, stat) {
    if (!err) {
        var access_time = (new Date()) - 1;
        fs.rename(process.cwd() + '/log/access.log', process.cwd() + '/log/' + access_time + '_access.log', function() {
            var accessLogStream = fs.createWriteStream(process.cwd() + '/log/access.log', {
                flags: 'a'
            });
            app.use(morgan('gamilms', 'combined', {
                stream: accessLogStream
            }));
            app.use(function keyword_filter(req, res, next) {
                if (/^(\/gamilms|\/assets|\/favicon\.ico|\/robots\.txt)/.test(req.path)) {
                    next();
                } else {
            ...
            var regex_app_path = new RegExp('/' + server_config.project_name + '\\\/app_[0-9]+\/(lib\/|cordova.js)');
            app.get(regex_app_path, function get_ionic_lib(req, res, next) {
                var req_path = req.path.replace('/' + server_config.project_name + '/app_', '').replace(/[0-9]+\//, '\/');
                var regex_app_path = new RegExp('/' + server_config.project_name + '\\\/app_[0-9]+\/(lib\/|cordova.js)');
