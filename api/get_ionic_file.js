...
module.exports = function(passport, Models) {
    var user_ionic = function(req, res, Models, next) {
        var req_path = req.path.replace(project_name + '/app_', '').replace(/[0-9]+\//, '\/');
                        ...
                        if (result.files[i].file_name === req_path) {
                            switch (true) {
                                case (!!regex_img.exec(result.files[i].file_name)):
                                    var imageData = new Buffer(result.files[i].file_content, 'base64');
                                    return res.send(imageData);
                                    break;
                                case (!!regex_css.exec(result.files[i].file_name)):
                                    response = querystring.unescape(result.files[i].file_content);
                                    if (response === '') {
                                        response = ' ';
                                    }
